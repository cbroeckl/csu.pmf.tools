...
) 
{
  ...
  #   feature.name = ramclustObj$featnames,
  #   xcmsOrd = ramclustObj$xcmsOrd,
  #   mz = ramclustObj$fmz,
  #   rt = ramclustObj$frt, 
  ...
  #       prcr <- which(ms[, "adduct"] %in% "[M+H]+")
  #       if(length(prcr) == 0) {prcr <- which(ms[, "adduct"] %in% "[M+NH4]+")}
  #       if(length(prcr) == 0) {prcr <- which(ms[, "adduct"] %in% ads)}
  ...
  #     for(i in 1:length(findmain[[cl]]$details)) {
  #       ms <- findmain[[cl]]$details[[i]]
  #       ms$charge[which(is.na(ms$charge))] <- 1
  #       ms$m <- ms$mz * ms$charge
  ...
  #       prcmz <- ms[prcr, "mz"]
  #       prctype <- ms[prcr, "adduct"]
  #       prcisogr <- ms[prcr, "isogr"]
  #       out <- paste(">compound ", ramclustObj$cmpd[cl], 
  #                    "\n", ">parentmass ", prcmz, "\n", ">ionization ", 
  #                    prctype, "\n", "\n", sep = "")
  #       ms.iso <- ms[which(ms$isogr == prcisogr), ]
  #       msms <- ms[which(ms$mz < prcmz),]
  #       if(any(!is.na(msms$adduct))) {
  #         isogr.rem <- msms$isogr[which(!is.na(msms$adduct))]
  #         msms <- msms[!msms$isogr %in% isogr.rem, ]
  #       }
  #       msms <- msms[((msms$iso == 0) | (is.na(msms$iso))), ]
  #       out <- paste(out, ">ms1peaks", "\n", sep = "")
  #       for (j in 1:nrow(ms.iso)) {
  #         out <- paste(out, ms.iso[j, 1], " ", ms.iso[j, 2], "\n", 
  #                      sep = "")
  #       }
  #       
  #       out <- paste(out, ">ms1peaks", "\n", sep = "")
  ...
  #       if (nrow(msms) > 0) {
  #         out <- paste(out, "\n", ">collision 25 ", "\n", sep = "")
  #         for (j in 1:nrow(msms)) {
  #           out <- paste(out, msms[j, 1], " ", msms[j, 
  #                                                   2], "\n", sep = "")
  #         }
  #         
  #         
